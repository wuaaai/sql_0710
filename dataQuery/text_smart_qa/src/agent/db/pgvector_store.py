...
这个模块替代原先适配 Vastbase floatvector 的实现，统一使用 PostgreSQL
的 pgvector 扩展来完成：
1. 向量表构建
2. 文档切片写入
3. 相似度检索
...
class PgVectorStore(VectorStore):
    ...
    def create_collection(self) -> None:
        ...
        由于向量维度要等首次生成 embedding 后才知道，因此真正的建表动作
        放在 `add_documents` 里完成。
        """
        ...
        **kwargs,
        ...
        with Session(self._engine) as session:
            for start in range(0, len(documents), batch_size):
                ...
                for text_value, embedding_value, metadata_value in zip(
                    texts[start:end],
                    embeddings[start:end],
                    metadatas[start:end],
                ):
                    row = TableCls(
                        c_document=text_value,
                        c_embedding=embedding_value,
                        c_metadata=metadata_value,
                    )
                    session.add(row)
                    session.flush()
    ...
    def add_texts(
        ...
        **kwargs,
        ...
        """兼容 LangChain 接口，支持直接写入文本数组。"""
        if metadatas is None:
            metadatas = [{} for _ in texts]
        documents = [Document(page_content=text_item, metadata=meta) for text_item, meta in zip(texts, metadatas)]
        return self.add_documents(documents, **kwargs)

    @classmethod
    def from_texts(cls, texts, embedding, metadatas=None, **kwargs):
        """禁止使用 LangChain 的快捷构造，统一走显式初始化。"""
